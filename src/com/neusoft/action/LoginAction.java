...
public class LoginAction extends ActionSupport {
   ...
   //退出
	public String tuichu() {
		if(role==1){
			ActionContext ac = ActionContext.getContext();//是Action执行时的上下文
		...
		if(role==1){
			...
			if(personList!=null && personList.size()>0){
				if(personList.size()<=8){
					...
					ActionContext.getContext().put("personList", personList.subList(0, 8));
				}
			}else{
				ActionContext.getContext().put("personList", personList);
			}
			...
			if(businessList!=null && businessList.size()>0){
				if(businessList.size()<=8){
					...
					ActionContext.getContext().put("businessList", businessList.subList(0, 8));
				}
			}else{
				ActionContext.getContext().put("businessList", businessList);
			}
